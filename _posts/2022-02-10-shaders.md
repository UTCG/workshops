...
We can make other colours by putting different values into the R/G/B parts of `col`, e.g. <span style="color:#FF8080 !important;">`red = vec3(1., .5, .5)`</span> or <span style="color:#4080FF !important;">`blue = vec3(.25, .5, 1.)`</span>.
