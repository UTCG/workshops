...
To learn the basics, let's start by rendering the [Mandelbrot set](https://en.wikipedia.org/wiki/Mandelbrot_set). Here's some [starter code]([https://www.shadertoy.com/view/NdsyDX](https://www.shadertoy.com/view/NdsyDX)) - we won't be paying much attention to the math, so the `mandel()` function is provided (and you don't need to understand it).
